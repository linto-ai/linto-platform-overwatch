...
class MongoController {
  ...
  async init(urlMongo) {
    ...
    this.clientDb = await this.client.db(process.env.LINTO_STACK_MONGODB_DBNAME)
  ...
  update(dataLintos, dataLog) {
    ...
    try {
      this.clientDb.collection(process.env.LINTO_STACK_MONGODB_COLLECTION_LINTOS).updateOne({
      ...
      this.clientDb.collection(process.env.LINTO_STACK_MONGODB_COLLECTION_LOG).insertOne({
