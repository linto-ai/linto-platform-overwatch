...
class LintoWebUsersModel extends MongoModel {
    ...
    async checkSlot(tokenData, sn) {
        if (tokenData && tokenData.data && tokenData.data.sessionId === sn) {
            let data = tokenData.data
            await this.findOne({ originUrl: data.originUrl })
                .then(webapp => {
                    webapp.applications.find(app => {
                        if (app.applicationId === data.applicationId && SlotsManager.getSn(sn)) {
                            console.log('Slot is taken by these user')
                        }
                    })
                })
        }
    ...
    async takeApplicationSlotOnLogin(originUrl, app, sn) {
        if (app.maxSlots > SlotsManager.countSlotsApplication(app.applicationId)) {
            SlotsManager.takeSlot(sn, { originUrl, applicationId: app.applicationId })
            return true
        } else return false
    }
