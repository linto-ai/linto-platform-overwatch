    ...
    return new Promise((resolve, reject) => {
      ...
      this.client.on('message', async (topic, payload) => {
        try {
          ...
          switch (_etat) {
            case 'status':
              if (_sn.indexOf('WEB_') !== -1) {
                const jsonPayload = JSON.parse(payload)
                if (jsonPayload.connexion === "online" && jsonPayload.auth_token !== undefined) {
                  MongoWebCollection.getSlot(jwtDecode(jsonPayload.auth_token.split('WebApplication')[1]), _sn)
                } else if (jsonPayload.connexion === "offline") {
                  debug('offline detected')
                  MongoWebCollection.deleteSlot(_sn)
                }
